...
async def execute_code_with_input(language_info: dict, code: str, stdin: str) -> dict:
    docker_image = language_info["docker_image"]
    filename = language_info["filename"]
    compile_cmd = language_info["compile_cmd"]
    run_cmd = language_info["run_cmd"]
    ...
    # Base64 encode the code and input to pass it safely through the shell
    encoded_code = base64.b64encode(code.encode('utf-8')).decode('utf-8')
    ...
    if compile_cmd:
        command = (
            ...
            f"cd /tmp && "
            f"({compile_cmd}) && "
            f"(echo {encoded_stdin} | base64 -d | {run_cmd})'"
        )
    else:
        command = (
            f"sh -c 'echo {encoded_code} | base64 -d > /tmp/{filename} && "
    ...
    try:
        ...
        if timed_out:
            ...
            # Check stderr even on exit code 0
            if stderr:
                error_msg = stderr
        ...
        logger.error(f"Error during code execution with input: {e}")
        return {
            "success": False,
            "output": "",
            "error": f"Execution Engine Error: {str(e)}",
